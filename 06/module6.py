...
def softmax(x):
    ...
    if x.ndim == 1:  # Handle a single image (1D array)
        return exp_x / np.sum(exp_x)
    return exp_x / np.sum(exp_x, axis=1, keepdims=True)

# Load the weights and biases from the pickle file
try:
    with open('RishiD_weights.pkl', 'rb') as f:
        params = pickle.load(f)
except FileNotFoundError:
    print("Error: Model file 'RishiD_weights.pkl' not found.")
    sys.exit(1)
...
try:
    ...
    print(f"Error: Missing parameter {e} in the model file.")
    sys.exit(1)
...
def predict_image(image_path, w1, b1, w2, b2):
    ...
    # Return the predicted class and processed image for visualization
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Error: The provided true digit should be an integer.")
        sys.exit(1)
